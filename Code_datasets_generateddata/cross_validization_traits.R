...
cross_validization <- function(U, A, traits, nMW, num_division, lambda_options, nPlants){

	Zero = array(0, dim = c(dim(traits)[2], 1))
...
#		6, 6, 4, 4, 3, 5, 7, 2, 1, 1, 2, 7, 6, 5)
	...
	for (div in 1:num_division){
		...
		for (lambda in 1:num_lambda){
			print(c("\t", lambda))
			W = optim(Zero, likelihood_traits, gr = NULL, U_new, A_new, traits, lambda_options[lambda], nMW-length(watches), nPlants, method = "BFGS")
			LL = likelihood_traits(W$par, U_left, A_left, traits, lambda_options[lambda], length(watches), nPlants)
			if(LL < min_LL){
				min_LL = LL
				vote_lambda = lambda
			}
