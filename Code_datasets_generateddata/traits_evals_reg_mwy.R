...
for (i in 1:3){
	...
	for (j in 1:num_flip_options){
		...
		for (l in 1:trials){
			...
			cor_data_kendall[j,i,l] = cor(Weight_test, result$par, method= "kendall")
			cor_data_spearman[j,i,l] = cor(Weight_test, result$par, method= "spearman")
			cor_data_pearsons[j,i,l] = cor(Weight_test, result$par, method= "pearson")
...
filename = paste("MWYtraitsCVno_", as.character(year), ".RData", sep = '')
